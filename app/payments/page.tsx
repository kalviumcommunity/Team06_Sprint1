...
import { CreditCard, Calendar, RefreshCw, Loader2, AlertCircle, CheckCircle } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur-lg dark:border-slate-800 dark:bg-slate-900/90 shadow-sm px-4 sm:px-8 py-3.5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-extrabold text-emerald-700 dark:text-emerald-400">My Payments</h1>
        </div>
        <ThemeToggle />
      </header>
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Payment Transactions</h2>
          <p className="mt-1 text-slate-500 dark:text-slate-400">
            View status and history of your subscription billing.
          </p>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center py-24 gap-3">
            ...
            <p className="text-slate-500 dark:text-slate-400">Loading your payments…</p>
          </div>
        ) : error ? (
          ...
          <div className="py-24 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Payments will appear once your orders are generated.</p>
          </div>
        ) : (
