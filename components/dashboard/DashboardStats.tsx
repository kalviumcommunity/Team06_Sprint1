  ...
  return (
    <>
      {items.map((item) => (
        <article
          key={item.title}
          className="group rounded-3xl border border-slate-200/80 bg-white p-4 shadow-[0_10px_40px_-20px_rgba(15,23,42,0.35)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_50px_-20px_rgba(16,185,129,0.45)] dark:border-slate-800 dark:bg-slate-900"
        >
          ...
          {item.actionLabel && item.actionHref && (
            <a href={item.actionHref} className="mt-2 inline-block text-sm font-semibold text-emerald-600 transition hover:text-emerald-700">
              {item.actionLabel} →
            </a>
          )}
        </article>
